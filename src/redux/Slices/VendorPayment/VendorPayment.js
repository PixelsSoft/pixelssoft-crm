...
export const AddVendorPayment = (data, token, reset) => async (dispatch) => {
  try {
    const response = await VendorPaymentServices.CreateVendorPayment(
      data,
      token
    );
...
export const GetVendorById = (id, token) => async (dispatch) => {
  try {
    if (id) {
      const response = await VendorPaymentServices.GetVendorPaymentById(
        id,
        token
      );
...
export const UpdateVendorPayment = (data, token, reset) => async (dispatch) => {
  try {
    const response = await VendorPaymentServices.UpdateVendorPayment(
      data,
      token
    );
    if (response?.status === 200) {
...
export const { SingleVendor, VendorPayments } = VendorPayment.actions;
