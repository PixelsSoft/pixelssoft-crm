...
const CreateInvoice = () => {
  ...
  const [price, setPrice] = useState(445);
  ...
  const { token, user, category, loading } = useSelector((state) => ({
  ...
  const handleSubmit = async (e) => {
    ...
    fromDate.append("customer_email", customerEmail);
    fromDate.append("customer_full_name", customerName);
    fromDate.append("customer_address", address);
    fromDate.append("customer_country", country);
    fromDate.append("customer_phone", phoneNumber);
    fromDate.append("project_categories_id", projectCategory);
    ...
    const response = await dispatch(AddInvoice(fromDate, token, reset));
  ...
  const dueDateFuc = (e) => {
    ...
    if (selectedDate < today && selectedDate > invoiceDate) {
      ...
      toast.error("Due date cannot be older than today.", {
        position: toast.POSITION.TOP_RIGHT,
      });
    } else {
  ...
  const invoiceFuc = (e) => {
    ...
    if (selectedDate < today) {
      ...
      toast.error("Invoice date cannot be older than today.", {
        position: toast.POSITION.TOP_RIGHT,
      });
    } else {
              ...
              <Form onSubmit={handleSubmit}>
                ...
                <Row className="mb-3">
                  <Form.Group as={Col} controlId="formGridState">
                    ...
                    <Form.Select
                      defaultValue="Choose..."
                      onChange={handleSelectProjectCategory}
                    >
