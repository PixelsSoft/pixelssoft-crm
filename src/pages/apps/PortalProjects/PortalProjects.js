...
export default function PortalProjects() {
  ...
  const { project, loading, token, roles } = useSelector((state) => ({
  ...
  const gettingProject = async () => {
    try {
      ...
      await dispatch(GetPortalProject(token));
  ...
  const ActionColumn = ({ row }) => {
    return (
      <React.Fragment>
        <Link
          to={`/apps/portalProjects/Profile/${row.original.id}`}
