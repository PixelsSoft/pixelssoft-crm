...
export default function Leads() {
  ...
  const { token, platforms, loading, leads, roles } = useSelector((state) => ({
  ...
  const gettingLeads = async () => {
    try {
        dispatch(startLoading());
        await dispatch(GetLead(token));
  ...
  return loading ? (
    ...
    <>
      ...
      <Modal size="lg" show={view} onHide={toggleView}>
        <Modal.Header closeButton>
